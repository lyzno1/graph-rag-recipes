...
\begin{enumerate}
    ...
    \item 建立了统一的配置注入机制，集中管理模型参数（默认 OpenAI gpt-4o-mini）
...
\begin{itemize}
    ...
    \item 暴露 LLM Provider/模型配置字段，默认使用 OpenAI gpt-4o-mini
\end{itemize}
...
\begin{itemize}
    \item \textbf{图构建时间}：约 300 条菜谱在秒级完成（O(N\textsuperscript{2}) 复杂度）
    \item \textbf{向量索引构建}：首次加载模型约 3-5 秒，后续复用
    \item \textbf{单次推荐响应}：图检索/向量检索均为毫秒级，LLM 生成 1-2 秒
\end{itemize}
...
# 3. 准备数据
uv run scripts/bootstrap_data.py --limit 0 --force-processed
...
\begin{enumerate}
    \item 设计了模块化的配置管理系统，支持环境变量注入并集中管理模型参数
